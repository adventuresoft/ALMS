            ...
            <div class="row mb-2 align-items-center">
                <div class="col-sm-6">
                    <h1 class="font-weight-bold text-dark mb-0">
                        <i class="fas fa-file-contract text-success mr-2"></i>ঋণ রিপোর্ট / Loan Report
                    </h1>
                </div>
        ...
        <div class="container-fluid">
            ...
            <div class="card report-card mb-4 no-print form-filter-sec">
                ...
                <div class="card-body p-4 bg-light">
                    <form method="GET" action="{{ route('report.loan-report') }}">
                        <div class="row">
                        ...
                        <div class="row">
                            <div class="col-md-12 text-right">
                                <a href="{{ route('report.loan-report') }}" class="btn btn-outline-secondary px-3 mr-2" style="border-radius: 6px;">
                                    <i class="fas fa-undo mr-1"></i> রিসেট (Reset)
                                </a>
                                <button class="btn btn-success px-4 font-weight-bold" type="submit" style="background-color: #006a4e; border-radius: 6px;">
            ...
            @if (isset($loans) && count($loans) > 0)
                ...
                <div class="row mb-4 no-print">
                    <div class="col-md-4 col-sm-6 mb-2">
                        <div class="kpi-card" style="background: linear-gradient(135deg, #0e7490 0%, #155e75 100%);">
                            <div class="kpi-title"><i class="fas fa-users mr-1"></i> মোট লোন সংখ্যা (Total Loans)</div>
                            <div class="kpi-value">{{ number_format($loans->total()) }} টি</div>
                        </div>
                    </div>
                    <div class="col-md-4 col-sm-6 mb-2">
                        <div class="kpi-card" style="background: linear-gradient(135deg, #047857 0%, #065f46 100%);">
                            <div class="kpi-title"><i class="fas fa-money-bill-wave mr-1"></i> মোট লোনের পরিমাণ (Total Amount)</div>
                            <div class="kpi-value">৳ {{ number_format($loans->sum('amount'), 2) }}</div>
                        </div>
                    </div>
                    <div class="col-md-4 col-sm-6 mb-2">
                        <div class="kpi-card" style="background: linear-gradient(135deg, #4338ca 0%, #3730a3 100%);">
                        ...
                        <table class="table table-hover table-striped table-report mb-0">
                            <thead>
                                <tr>
                                    ...
                                    <th width="18%">যোগাযোগ (Mobile/Email)</th>
                                    <th width="17%">ঠিকানা / এলাকা</th>
                                    <th width="18%">ব্যাংক ও শাখা</th>
                                    <th width="12%">অর্থবছর</th>
                                    <th width="10%">লোনের পরিমাণ</th>
                                    <th class="text-center" width="8%">স্ট্যাটাস</th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach ($loans as $key => $loan)
                                    @if($loan && isset($loan->user))
                                    <tr>
                                        <td class="text-center font-weight-bold">{{ $loans->firstItem() + $key }}</td>
                                        <td>
                                            <div class="font-weight-bold text-dark" style="font-size: 14px;">{{ $loan->user->name ?? '---' }}</div>
                                            <div class="mt-1">
                                                <span class="badge-approved-id">
                                                    <i class="fas fa-id-badge mr-1"></i>{{ $loan->user->approved_id ?? $loan->user->system_id ?? 'N/A' }}
                                                </span>
                                            </div>
                                        </td>
                                        <td>
                                            @if($loan->user->mobile)
                                                <div class="text-dark font-weight-bold mb-1">
                                                    <i class="fas fa-phone-alt text-success mr-1"></i>{{ $loan->user->mobile }}
                                                </div>
                                            @endif
                                            @if($loan->user->email)
                                                <div class="text-muted small">
                                                    <i class="fas fa-envelope text-primary mr-1"></i>{{ $loan->user->email }}
                                                </div>
                                            @endif
                                        </td>
                                        <td>
                                            <span class="text-secondary">
                                                {{ $loan->user->addressInfo->permanent_area ?? $loan->user->addressInfo->present_area ?? 'N/A' }}
                                            </span>
                                        </td>
                                        <td>
                                        ...
                                        <td class="text-center">
                                            ...
                                            @else
                                                <span class="badge badge-warning px-2 py-1"><i class="fas fa-clock mr-1"></i>{{ ucfirst($loan->status ?? 'Pending') }}</span>
                                            @endif
                                        </td>
                                    </tr>
                                    @endif
            ...
            @else
                <div class="card report-card p-5 text-center my-4">
                    <div class="text-muted">
                        ...
                        <h5 class="font-weight-bold">কোনো লোন রিপোর্ট ডাটা পাওয়া যায়নি</h5>
                        <p class="mb-0">আপনার সিলেক্টকৃত অর্থবছর বা ব্যাংকের ফিল্টারে কোনো তথ্য সংরক্ষিত নেই। অনুগ্রহ করে অন্য ফিল্টার চেষ্টা করুন।</p>
                    </div>
                </div>
            @endif
