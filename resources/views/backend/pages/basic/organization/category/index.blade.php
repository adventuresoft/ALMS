@extends('backend.master', ['mainMenu' => 'Basic', 'subMenu' =>'OrganizationCategory'])
@push('style')
@endpush
                        ...
                        <div class="card-header">
                            <div class="row">
                                <div class="col-md-6 text-left">
                                    <h3 class="card-title">Organization Category List</h3>
                                </div>
                                <div class="col-md-6 text-right">
                                    {{-- {{route('death.create')}} --}}
                                    <a href="{{route('basic-settings.organization-category.create')}}" class="btn btn-primary">Create</a>
                                </div>
                                    ...
                                    <tr>
                                      ...
                                      <td>
                                        <div class="table-action">
                                            @can('organization-category-update')
...
@endcan
                                            ...
                                            @can('organization-category-delete')
<form class="deleteCategory" method="post">
                                              @csrf
                                              @method('DELETE')
                                              <input type="hidden" class="id" name="id" value="{{$item->id}}">
                                              <input type="hidden" class="deleteUrl" name="deleteUrl" value="{{route('basic-settings.organization-category.destroy', $item->id)}}">
			...
			onShown: function (toast) {
				$("#confirmationRevertYes").click(function(){
                    ...
                    success: function (response) {
                        ...
                        setTimeout(function() {
                            location.href = "{{route('basic-settings.organization-category.index')}}";
                        }, 2000)
                    },
